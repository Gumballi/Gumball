...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            rishuresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            rishufile = await event.client.download_media(rishuresponse, "./temp")
            return await unzip(rishufile)
