...
async def convert_to_image(event, client):
    rishu = await event.get_reply_message()
    if not (
            rishu.gif
            or rishu.audio
            or rishu.voice
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if rishu and rishu.photo:
        rishu_final = downloaded_file_name
    elif rishu.sticker and rishu.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        rishu_final = image_name20
    elif rishu.sticker and rishu.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            ...
            return
        rishu_final = imgpath
    elif rishu.gif or rishu.video or rishu.video_note:
...
def tgs_to_gif(sticker_path: str, quality: int = 256) -> str:                  
    semx = os.path.join(dwlpath, "THANOSPROtgs.gif")
