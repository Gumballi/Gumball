...
@rishu_cmd(pattern="ping$")
async def pong(rishu):
    ...
    if a:
        ...
        PIC = None
    event = await eor(rishu, "`·.·★ ℘ıŋɠ ★·.·´")
    cid = await client_id(event)
    ultronceo, rishu_USER = cid[0], cid[1]
    rishu_mention = f"<a href='tg://user?id={ultronceo}'>{rishu_USER}</a>"
    ...
    if PIC:
        await event.client.send_file(event.chat_id,
                                     file=PIC,
                                     caption=ping_txt.format(ms, uptime, rishu_mention),
                                     parse_mode="HTML",
                                 )
        await event.delete()
    else:
        await event.edit(ping_txt.format(ms, uptime, rishu_mention), parse_mode="HTML")


@rishu_cmd(pattern="limits$")
...
@rishu_cmd(pattern="dc$")
async def _(event):
    result = await event.client(functions.help.GetNearestDcRequest())
    await eor(event, result.stringify())


@rishu_cmd(pattern="config$")
async def _(event):
...
@rishu_cmd(pattern="vars(?:\s|$)([\s\S]*)")
async def lst(event):
    flag = (event.text[6:9]).lower()
    if flag and flag == "-db":
        rishu = await eor(event, "Getting DB variables..")
        dbx = "**• List of DB Variables:** \n\n"
        for data in db_config:
            dbx += f"» `{data}`\n"
        await rishu.edit(dbx)
    else:
        rishu = await eor(event, "Getting configs list...")
        osx = "**• List of OS Configs:** \n\n"
        for data in os_config:
            osx += f"» `{data}`\n"
        await rishu.edit(osx)


@rishu_cmd(pattern="schd(?:\s|$)([\s\S]*)")
async def _(event):
...
CmdHelp("bot").add_command(
    ...
    "ping", None, "Checks the ping speed of your Շђคภ๏ร-קг๏"
).add_command(
    "schd", "<secs> - <message>", "Sends your message in given secs", "schd 10 - rishuo"
).add_command(
