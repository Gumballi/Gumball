...
@rishu_cmd(pattern="stog(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
        return await eod(event, "Reply to animated sticker to make gif.")
    rishu = await eor(event, "Converting...")
    if event.pattern_match.group(1):
        quality = event.pattern_match.group(1)
    else:
        quality = 512
    rply = await event.get_reply_message()
    rishu_ = await event.client.download_media(rply.media)
    gifs = tgs_to_gif(rishu_, quality)
    await event.client.send_file(event.chat_id, file=gifs, force_document=False)
    await rishu.delete()


@rishu_cmd(pattern="stoi$")
async def _(rishu):
    reply_to_id = rishu.message.id
    if rishu.reply_to_msg_id:
        reply_to_id = rishu.reply_to_msg_id
    event = await eor(rishu, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await rishu.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            await rishu.client.send_file(
                event.chat_id,
