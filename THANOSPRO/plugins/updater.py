...
THANOSPRO_info = "https://raw.githubusercontent.com/rishabh87yhtts/THANOS-PRO/master/THANOSPRO-info.json"
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
requirements_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "requirements.txt")


async def rishu_info(THANOSPRO_info):
    infos = requests.get(THANOSPRO_info).json()
    _version = infos['THANOSPRO-INFO']['version']
    _release = infos['THANOSPRO-INFO']['release-date']
    _branch = infos['THANOSPRO-INFO']['branch']
    _author = infos['THANOSPRO-INFO']['author']
    _auturl = infos['THANOSPRO-INFO']['author-url']
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await eor(event, "✅ Successfully updated Շђคภ๏ร-קг๏!\n\nBot is restarting please wait for a minute.")
    args = [sys.executable, "-m", "THANOSPRO"]
    os.execle(sys.executable, *args, os.environ)
    return


@rishu_cmd(pattern="update(| now)$")
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        _version, _release, _branch, _author, _auturl = await rishu_info(THANOSPRO_info)
        output_ = f"**Your Bot Version :** `{rishu_ver}` \n**Owner :** {rishu_mention} \n\n**Official THANOSPRO Version :** `{_version}` \n**Release Date :** `{_release}` \n**Official Repo Branch :** `{_branch}` \n**Update By :** [{_author}]({_auturl})"
        if str(_version) not in str(rishu_ver):
            output_ += f"\n\n**Do** `{hl}update build` **to update your THANOSPRO to latest version.**"
        await event.edit(output_)
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your **Շђคภ๏ร-קг๏** !!")

    if force_update:
        await event.edit(
            "`Force-Updating Շђคภ๏ร-קг๏. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your Շђคภ๏ร-קг๏ Is UpToDate**\n\n**Version :**  __{rishu_ver}__\n**Oɯɳҽɾ :**  {rishu_mention}")
    else:
        await event.edit("**Please set up**  `HEROKU_API_KEY`  **from heroku to update!**")
    return


@rishu_cmd(pattern="update build$")
async def upstream(event):
    event = await eor(event, "`Hard-Update In Progress... \nPlease wait until docker build is finished...`")
    off_repo = "https://github.com/thanosuser/THANOS-PROS"
    os.chdir("/app")
    git_rishu = f"rm -rf .git"
    try:
        await runcmd(git_rishu)
    except BaseException:
        pass
    txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    try:
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your Շђคภ๏ร-קг๏. Basically if you restart dyno it will go back to previous deploy."
).add_command(
  "update build", None, "Hard-Update Your Շђคภ๏ร-קг๏. This won't take you back to your previous deploy. This will be triggered even if there is no changelog."
).add_info(
  "Շђคภ๏ร-קг๏ Updater."
).add_warning(
  "✅ Harmless Module."
