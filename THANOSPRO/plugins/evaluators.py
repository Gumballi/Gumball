...
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        ...
        rishuevent = await eor(event, "`Executing.....`")
        process = await asyncio.create_subprocess_shell(
            cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        stdout, stderr = await process.communicate()
        result = str(stdout.decode().strip()) + str(stderr.decode().strip())
        rishuuser = await event.client.get_me()
        if rishuuser.username:
            curruser = rishuuser.username
        else:
            curruser = "@thanospros"
        uid = os.geteuid()
        if uid == 0:
...
@rishu_cmd(pattern="eval(?:\s|$)([\s\S]*)")
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        cmd = "".join(event.text.split(maxsplit=1)[1:])
        if not cmd:
            return await eod(event, "`What should i run ?..`")
        rishuevent = await eor(event, "`Running ...`")
        ...
        if len(final_output2) > 4092:
            await eor(rishuevent, final_output, deflink=True, linktext=f"**•  Eval :** \n`{cmd}` \n\n**Pasted:** ")
        else:
            await eor(rishuevent, final_output, deflink=True, linktext=f"{final_output2} \n\n**Also Pasted** ")
    else:
        await eod(event, f"**Eval Is Disbaled !!** \n\n__Do__ `{hl}svar USE_EVAL TRUE` __to enable eval commands.__")
...
CmdHelp("evaluators").add_command(
  "eval", "<expr>", "Execute a python script", "eval print('rishuo World.')"
).add_command(
  "exec", "<command>", "Execute a Terminal command on THANOSPRO server and shows details"
).add_command(
