...
async def spammer(event):
    ...
    rishu = await eor(event, f"Spamming {counter} times...")
...
@rishu_cmd(pattern="bigspam(?:\s|$)([\s\S]*)")
async def bigspam(event):
    lg_id = Config.LOGGER_ID
    msg_ = event.text[9:]
    THANOSPRO_count = int(msg_.split(" ")[0])
    reply_msg = await event.get_reply_message()
    if reply_msg:
        rishu_spam = reply_msg
    else:
        rishu_spam = msg_.replace(str(THANOSPRO_count), "")
...
# Special Break Spam Module For THANOSPRO Made By Chirag Bhargava.
# Team THANOSPRO
@rishu_cmd(pattern="bspam(?:\s|$)([\s\S]*)")
async def spammer(event):
    lg_id = Config.LOGGER_ID
    msg_ = event.text[7:]
    counter = int(msg_.split(" ")[0])
...
CmdHelp("spam").add_command(
  "spam", "<number> <text>", "Sends the text 'X' number of times.", "spam 99 rishuo"
).add_command(
  "mspam", "<reply to media> <number>", "Sends the replied media (gif/ video/ sticker/ pic) 'X' number of times", "mspam 100 <reply to media>"
).add_command(
  "dspam", "<delay> <spam count> <text>", "Sends the text 'X' number of times in 'Y' seconds of delay", "dspam 5 100 rishuo"
).add_command(
  "uspam", "<reply to a msg> or <text>", "Spams the message unlimited times until you get floodwait error.", "uspam rishuo"
).add_command(
  "bspam", "<count> <text or reply>", "Spams the message X times without floodwait. Breaks the spam count to avoid floodwait.", "bspam 9999 rishuo"
).add_command(
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what THANOSPRO iz known for. The Best BigSpam Ever", "bigspam 9999 rishuo"
).add_info(
  "Spammers Commands"
).add_warning(
