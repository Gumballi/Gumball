...
async def get_media(event):
    ...
    rishu = await eor(event, "Downloading Media From this Channel.")
    ...
    await rishu.edit("Downloaded " + output + " files.")


@rishu_cmd(pattern="geta ([\s\S]*)")
async def get_media(event):
    dir = "./channel_dwl/"
    try:
        os.makedirs("./channel_dwl/")
...
CmdHelp("chnl_dwl").add_command(
  ...
  "getc", "<limit> <channel username>", "will download latest given number of media from channel into your bot server", "getc 1000 @thanospros"
).add_info(
