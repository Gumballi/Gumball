...
rishu_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"


@rishu_cmd(pattern="unblock$")
async def unb(event):
    if event.is_private:
...
async def approve_p_m(event):
    if event.is_private:
        ...
        if str(reply_s.sender_id) in DEVLIST:
            await eod(event, "**I can't Block My boss rishabh anand!!**")
            return
        if pm_sql.is_approved(event.chat_id):
            pm_sql.disapprove(event.chat_id)
...
if PM_ON_OFF != "DISABLE":
    ...
    async def dapprove(event):
        if event.is_private:
            ...
            firstname = replied_user.user.first_name
            if str(event.chat_id) in DEVLIST:
                await eod(event, "**I can't  this is my boss!!**")
                return
            if pm_sql.is_approved(event.chat_id):
                pm_sql.disapprove(event.chat_id)
                await eod(event,
    ...
    async def on_new_private_message(event):
        if not event.is_private:
            return
        cid = await client_id(event)
        ultronceo, rishu_mention = cid[0], cid[2]
        CSTM_PMP = gvarstat("CUSTOM_PMPERMIT") or "**You Have Trespassed To My Master's PM!\nThis Is Illegal And Regarded As Crime.**"
        rishu_FIRST = "**🔥 Շђคภ๏ร-קг๏ קгเשคՇє รєςยгเՇץ קг๏Շ๏ς๏ɭ 🔥**\n\nrishuo i am thanos !!  Welcome to {}'s PM. This is an automated message.\n\n{}".format(rishu_mention, CSTM_PMP)
        if event.sender_id == ultronceo:
            return
        if str(event.sender_id) in DEVLIST:
            return
        if Config.LOGGER_ID is None:
            await event.client.send_message(ultronceo, "Please Set `LOGGER_ID` For Working Of Pm Permit")
            return
        message_text = event.message.raw_text
        chat_ids = event.sender_id
        if rishu_FIRST == message_text:
            return
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        if PM_WARNS[chat_ids] == Config.MAX_SPAM:
            r = await event.reply(rishu_ZERO)
...
CmdHelp("pm_permit").add_command(
  ...
  "listapproved", None, "Sends the list of all users approved by Շђคภ๏ร-קг๏"
).add_info(
  "PM SECURITY"
).add_warning(
  "✅ Harmless Module."
